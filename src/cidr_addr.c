...
/*
 * hex_val - 256-entry hex digit lookup table.
 *
 * Valid entries hold 0..15. All other entries hold 0xFF as an invalid
 * sentinel. This keeps IPv6 hex-group parsing on the same RFC-only surface
 * while removing the per-character conditional chain from the hot path.
 */
	...
	if (last_colon != NULL && is_mixed_suffix(last_colon + 1)) {
		...
		while (p < hex_end) {
			...
			while (p < hex_end) {
				...
				dig = hex_val[(unsigned char)*p];
				if (dig == 0xFF)
					break;
				value = (uint16_t)(value * 16 + dig);
				digit_count++;
				p++;
			}
